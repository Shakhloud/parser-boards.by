...
def parse_single_banner(
    banner_data: dict, fetch_details: bool = False
...
def parse_boards_by(
    min_sides: int = TARGET_SIDES,
    fetch_details: bool = True,
    max_constructions: int = MAX_CONSTRUCTIONS,
    ...
    if max_constructions > 0:
        ...
        logger.info(
            f"Ограничение: парсим только {len(banners_to_parse)} из {total} конструкций"
        )
    else:
    ...
    for idx, (banner_id, banner_data) in enumerate(banners_to_parse.items(), 1):
        try:
            ...
            if idx % 25 == 0 or idx == len(banners_to_parse):
                logger.info(
                    f"Прогресс: {idx}/{len(banners_to_parse)} конструкций, {len(all_sides)} сторон"
                )

            construction, sides = parse_single_banner(
                banner_data, fetch_details=fetch_details
            )
    ...
    logger.info(
        f"Распаршено: {len(constructions)} конструкций, {len(all_sides)} сторон"
    )
    ...
    try:
        ...
        for cons_data in constructions:
            ...
            existing = (
                ...
                .filter(Construction.gid == cons_data["gid"])
                .first()
            )
        ...
        session.commit()
        logger.info(
            f"Сохранено в БД: {len(gid_to_id)} конструкций, {saved_sides} сторон"
        )
        return saved_sides
...
def load_from_database() -> list[ConstructionSideExport]:
    ...
    try:
        ...
        sides = session.query(ConstructionSide).join(Construction).all()
...
def export_to_json(
    data: list[ConstructionSideExport], output_path: str = "result.json"
) -> None:
...
def export_to_xlsx(
    data: list[ConstructionSideExport], output_path: str = "result.xlsx"
) -> None:
    ...
    for item in data:
        ws.append(
            [
                ...
                (
                    "Да"
                    if item.lighting is True
                    else "Нет" if item.lighting is False else ""
                ),
                item.size,
                item.material,
            ]
